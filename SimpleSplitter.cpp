...
BOOL CSimpleSplitter::CreatePane(int nIndex, CRichEditCtrl * pPaneWnd) //HLSLEdit * pPaneWnd)
{
	CRect rcPane;
	...
	if (!pPaneWnd->Create(WS_CHILD | WS_VISIBLE | ES_AUTOVSCROLL | ES_AUTOHSCROLL |
...
void CSimpleSplitter::OnPaint() 
{
	...
	COLORREF clrBar = RGB(64,64,64); // ::GetSysColor(COLOR_BTNFACE);
	int i, x = 0;
